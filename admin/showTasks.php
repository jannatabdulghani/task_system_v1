          ...
          <label>Search:<input type="search" class="form-control form-control-sm" placeholder="" aria-controls="example1"></label></div></div></div><div class="row"><div class="col-sm-12"><table id="example1" class="table table-bordered table-striped dataTable dtr-inline" role="grid" aria-describedby="example1_info">
                  <thead>
                      <tr role="row">
                         ...
                         <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" aria-label="user: activate to sort column ascending">user</th>
              ...
              <tbody>
                <?php  while($tasks=mysqli_fetch_assoc($quryGetTasks)){?>
                    <tr>
                      <td><?= $tasks['title'];?></td>
                      <td><?= $tasks['name'];?></td>
                      <td><?php if( $tasks['taskStautes']==0){
                        echo"Pending";}
                        else{
                          echo "Approved";
                        }
                      ?></td>
                       <td>
                        ...
                        <i class="fa btn btn-danger fa-trash-alt" aria-hidden="true"></i>
                        </a>
                        <a href="handlers/approve_task.php?taskId=<?=$tasks['taskId']?>" class="p-2">
                        <i class="fa fa-check text-red bg-green p-2 border rounded" aria-hidden="true"></i>
                        </a>
                      </td>
                    </tr>
                     <?php
